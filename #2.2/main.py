...
# This is my audio link
AUDIO_LINK = "https://www.soundjay.com/button/button-1.mp3"
...
@bot.message_handler(commands=["audionet", "audnet"])
def audionet(message):
    download_file(AUDIO_LINK, "myaudio.mp3")
    audnet = open("myaudio.mp3", "rb")
    bot.send_audio(message.chat.id, audnet)


@bot.message_handler(commands=["sticker", "stckr"])
def sticker(message):
    bot.send_sticker(
        message.chat.id, "CAACAgIAAxkBAAEBlxdfs_9CGh0UPZTU4-QFVyYGzBTJtQAC8gADVp29ChCdi3ZTetJkHgQ")
